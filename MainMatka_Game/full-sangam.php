...
if (isset($_POST['single_submit']) && isset($_SESSION['usr_id'])!="") {
    if (!app_validate_csrf()) {
        echo "<script>window.location = 'full-sangam.php?invalidrequest';</script>";
        exit;
    }
	...
	if(get_SettingValue('pause_main_market_bidding_website')){
		...
		$user_id= $_SESSION['usr_id'];
            ...
            $open_digit = mysqli_real_escape_string($con,$_POST['open_digit']);
            $close_digit = mysqli_real_escape_string($con,$_POST['close_digit']);
            ...
            if(!$_scraped_bet_validated && !IsTempTestMarketGame($game_id) && time() >= $market_time){
                ...
                echo "<script>window.location = 'full-sangam.php?invalid_date&".$get_parameters."';</script>";
    				...
    				{
                        $placed = app_place_bets($user_id, $game_id, 'full_sangam', [$fs_digit => $amount], 0);
                    } else {

...
if (isset($_POST['single_submit']) && isset($_SESSION['usr_id'])!="") {
    if (!app_validate_csrf()) {
        echo "<script>window.location = 'single.php?invalidrequest';</script>";
        exit;
    }
	...
	if(get_SettingValue('pause_main_market_bidding_website')){
		...
		$user_id= $_SESSION['usr_id'];
            ...
            if ($scraped_id) {
                ...
                if (!$check['allowed']) {
                    echo "<script>alert('No Bets Taken! " . addslashes($check['reason']) . "')</script>";
                    echo "<script>window.location = 'index.php';</script>";
                    exit;
                }
            ...
            if(!$_scraped_bet_validated && !IsTempTestMarketGame($game_id) && time() >= $market_time){
                ...
                $placed = app_place_bets($user_id, $game_id, 'single', $bets, 0, $default_game, $_scraped_bet_date);
        ...
        <div id="content">
            ...
            <div class="card-full-page tb-10">
                    ...
                    foreach($single_digits as $digit){?>
                        ...
                        <div class="col-3">
                                    <div class="bidinputdiv">
                                        ...
                                        <input type="text" value="" class="pointinputbox" onclick="clickPanna(this)" id="single<?php echo $digit;?>" name="single<?php echo $digit;?>" readonly>
                                    </div>
                        </div>
                ...
                <div class="tb-10 text-center" id="winAmountBox" style="display:none;background:rgba(104,211,145,0.08);border:1px solid rgba(104,211,145,0.2);border-radius:10px;padding:10px;margin:10px 0;">
                    ...
                    <small style="color:rgba(255,255,255,0.4);">Rate: <?php echo get_RateSingle(); ?>x</small>
                </div>
                ...
                <script>
                (function(){
                    var rate = <?php echo (float) get_RateSingle(); ?>;
